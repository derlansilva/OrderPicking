...
export default function ItemsConferenceScreen({ route, navigation }) {
  ...
  const [modalMessage, setModalMessage] = useState('');
    ...
    function handleBarCodeRead(data) {
      console.log("codigo do item " ,data);
      if (data) {
        //setOrderNumber(data)
        handleScan(data);
      }
  ...
  const handleScan = async (code) => {
    ...
    const index = orderItems.findIndex(item => item.barCode === trimmedCode);
  ...
  const handleFinish = async () => {
    try {
      ...
      const response = await apiServices.changeOrderStatus(orderId, "finished");
